...
function App() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      axios.get("https://6263ce97c430dc560d31b618.mockapi.io/favorites").then(res => {
  ...
  const onAddToFavorite = async (obj) => {
    try {
      if(favorites.find((favObj) => favObj.id === obj.id)) {
        axios.delete(`https://6263ce97c430dc560d31b618.mockapi.io/favorites/${obj.id}`)
        // setFavorites((prev) => prev.filter((item) => item.id !== obj.id))
      }else {
        const {data} = await axios.post("https://6263ce97c430dc560d31b618.mockapi.io/favorites", obj)
